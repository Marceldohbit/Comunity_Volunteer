      ...
      <div className="container">
        ...
        <div className="communities-grid">
          {communities.map((community) => (
            <div key={community.id} className={`community-card fade-in ${community.isPending ? 'pending-approval' : ''}`}>
              <div className="community-icon">
                <i className={`fas ${community.icon}`}></i>
              </div>
              <h3>
                {community.name}
                {community.isPending && <span className="pending-badge">Pending Approval</span>}
              </h3>
