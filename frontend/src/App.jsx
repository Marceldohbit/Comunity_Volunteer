...
function App() {
  ...
  useEffect(() => {
    const allCities = [...new Set([...communities.map(c => c.location.split(',')[0]), ...works.map(w => w.location.split(',')[0])])];
    setCities(allCities);
  }, [communities, works]);
  ...
  const loadData = async () => {
    try {
      // Load regions
      const regionsResponse = await fetchAPI('/regions');
      ...
      // Load communities
      const communitiesResponse = await fetchAPI('/communities', filterParams);
      ...
      // Load works
      const worksResponse = await fetchAPI('/works', filterParams);
